...
describe("user object validation tests", () => {
  ...
  it("3. The user schema does not accept an invalid email", () => {
    const { error } = userSchema.validate(
      { name: "Bob", email: "invalid-email", password: "Test123!" },
      { abortEarly: false },
    );
    expect(
      error.details.find((detail) => detail.context.key == "email"),
  ...
  it("4. The user schema requires a password", () => {
    const { error } = userSchema.validate(
      { name: "Billy", email: "billy@email.com" },
      { abortEarly: false },
    );
    expect(
      error.details.find((detail) => detail.context.key == "password"),
  ...
  it("5. The user schema requires a name.", () => {
    const { error } = userSchema.validate(
      { email: "billy@bob.com", password: "Test123!" },
      { abortEarly: false },
    );
    expect(
  ...
  it("6. The name must be valid (3 to 30 characters).", () => {
    const { error } = userSchema.validate(
      { name: "Jo", email: "jojo@test.com", password: "Test123!" },
      { abortEarly: false },
    );
    expect(
      error.details.find((detail) => detail.context.key == "name"),
  ...
  it("7. If validation is performed on a valid user object, error comes back falsy", () => {
    const { error } = userSchema.validate(
      { name: "Bob", email: "billy@bob.com", password: "Test123!" },
      { abortEarly: false },
    );
...
describe("task validation tests", () => {
  ...
  it("9. If an isCompleted value is specified, it must be valid.", () => {
    const { error } = taskSchema.validate(
      { title: "test task title", isCompleted: "yes" },
      { abortEarly: false },
    );
    expect(
